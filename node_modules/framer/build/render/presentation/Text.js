...
import { useNavigate, NavigationRoutesContext } from "../../modules/useNavigate.js";
import { useIsOnFramerCanvas } from "../../modules/useIsOnFramerCanvas.js";
import { parseFramerPageLink, replaceFramerPageLinks } from "../../modules/framerPageLink.js";
import { useLinkMatchesRoute } from "../../components/NavigationRouteContext.js";
...
export const Text = React.forwardRef(function Text(props, forwardedRef) {
    ...
    const navigate = useNavigate({ preload: props.preload });
    const routes = React.useContext(NavigationRoutesContext);
    ...
    const interceptPageLinks = React.useCallback((event) => {
        ...
        const page = anchorElement.getAttribute("data-framer-page-link-target" /* Page */);
        if (page) {
            const transition = anchorElement.getAttribute("data-framer-page-link-transition" /* Transition */);
            const options = { transition: "instant" };
            if (transition)
                new URLSearchParams(transition).forEach((value, key) => (options[key] = value));
            event.preventDefault();
            return navigate(page, options);
        }
        // If for some reason the page links in the draft text have not been
        ...
        if (href) {
            ...
            if (!link || !link.target)
                return;
            event.preventDefault();
            return navigate(link.target, link.attributes);
        }
    }, [navigate]);
    useEffect(() => {
    ...
    const rawHTML = React.useMemo(() => {
        if (!props.rawHTML || onCanvas || !routes)
            return props.rawHTML;
        return replaceFramerPageLinks(props.rawHTML, routes);
    }, [props.rawHTML, routes, onCanvas]);
...
class TextComponent extends Layer {
    ...
    renderMain = (inCodeComponent) => {
        ...
        const { font, visible, alignment, willChangeTransform, opacity, id, layoutId, className, transition, variants, name, __fromCanvasComponent, _initialStyle, widthType, heightType, _usesDOMRect, autoSize, style: styleProp, fontLoadStatus, matchesCurrentRoute, } = this.props;
        const frame = this.frame;
        if (!visible) {
            return null;
        }
        ...
        if (rawHTML) {
            ...
            return (React.createElement(motion.div, { layoutId: layoutId, id: id, ...dataProps, style: style, transformTemplate: hasTransformTemplate ? transformTemplate(this.props.center) : undefined, dangerouslySetInnerHTML: { __html: rawHTML }, "data-center": this.props.center, className: cx(className, matchesCurrentRoute && "isCurrent"), transition: transition, variants: variants, ref: this.setElement }));
        }
    };
}
...
function findAnchorElementWithin(target, containerRef) {
